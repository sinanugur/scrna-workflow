...
rule dimplot_for_microbiome:
    ...
    shell:
        "workflow/scripts/scrna-microbiome-dimplot.R --rds {input.rds} --input.microbiome_rds {input.microbiome_rds} ----output.plot {output} --reduction.type {wildcards.reduction} --taxa {wildcards.taxa}"
