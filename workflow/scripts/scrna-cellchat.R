...
scrna@meta.data %>% dplyr::mutate(identity=paste0("cluster_",as.character(get(opt$idents)))) -> scrna@meta.data
...
pdf(opt$output.interactions.plot, width=5, height=5) 
netVisual_circle(cellchat@net$count, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Number of interactions")
netVisual_circle(cellchat@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Interaction weights/strength") 
dev.off()

pathToPlotsStep2ResolutionCellChatSignPath=opt$output.dir
...
n = length(allPathways)

netAnalysis_contribution(cellchat, signaling = allPathways,font.size = 9.5) & ggthemes::theme_few() -> p1

ggsave(plot =p1,filename=paste0(opt$output.dir,"all-pathways-contribution-ligand-receptor-pairs.pdf"),width=6,height=6+(n*0.15))
